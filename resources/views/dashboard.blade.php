    ...
    public function calculateStats()
    {
        ...
        $stats = match($user->role) {
            'super_admin' => [
                ...
                'pending_requests' => \App\Models\AssetRequest::where('status', 'pending')->count(),
                'maintenance_due' => \App\Models\Asset::whereDate('next_maintenance_date', '<=', now())->count(),
                'recent_audits' => \App\Models\Audit::latest()->take(5)->get(),
            ],
            'admin' => [
                'department_assets' => \App\Models\Asset::where('current_department_id', $user->department_id)->count(),
                'department_users' => \App\Models\User::where('department_id', $user->department_id)->count(),
                'pending_requests' => \App\Models\AssetRequest::where('status', 'pending')
                    ->whereHas('requester', fn($q) => $q->where('department_id', $user->department_id))
                    ->count(),
                'maintenance_due' => \App\Models\Asset::where('current_department_id', $user->department_id)
                    ->whereDate('next_maintenance_date', '<=', now())
                    ->count(),
            ],
            'auditor' => [
                'completed_audits' => \App\Models\Audit::where('auditor_id', $user->id)->count(),
                'pending_audits' => \App\Models\Asset::whereNull('last_audit_date')
                    ->orWhereDate('last_audit_date', '<=', now()->subMonths(3))
                    ->count(),
                'recent_audits' => \App\Models\Audit::where('auditor_id', $user->id)
                    ->latest()
                    ->take(5)
                    ->get(),
            ],
            'user' => [
                'assigned_assets' => \App\Models\Asset::where('assigned_to', $user->id)->count(),
                'pending_requests' => \App\Models\AssetRequest::where('requester_id', $user->id)
                    ->where('status', 'pending')
                    ->count(),
                'upcoming_returns' => \App\Models\Asset::where('assigned_to', $user->id)
